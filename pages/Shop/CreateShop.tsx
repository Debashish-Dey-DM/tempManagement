...
const CreateShop = () => {
  ...
  const mount = async () => {
    const res = await axios.get("localhost:3000/api/shop/GetNewShopId");
    setNewId(res.data);
  };
  useEffect(() => {
    mount();
  ...
  const submitData = async (e: any) => {
    ...
    const res = await axios.post("localhost:3000/api/shop/createShop", {
      rate,
    });
  ...
  return (
    ...
    >
      <Container className={`${commonStyles.commonForm} py-3`}>
        <div>
          <h3 className="alert alert-primary">দোকান তৈরি</h3>
